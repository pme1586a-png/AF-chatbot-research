...
/* Streamlit 버전에 관계없이 버튼 내부 왼쪽 정렬 */
...
2023 ACC/AHA/ACCP/HRS AF guideline 범위에서만
쉽고 간결한 한국어로 답한다.

진단, 처방, 약물 시작/중단/용량 변경,
개인별 시술 결정을 하지 않는다.
...
"""
            ...
            ans = (
                ...
                "교육 모듈의 고정 교육내용은 정상적으로 이용할 수 있습니다."
            )
...
# 교육 모듈 화면
    ...
    with answer_col:
        ...
        if st.session_state.question is None:
            ...
            if mid == "1" and i == 0:
                ...
                st.warning(
                    "🖼️ 정상 심전도와 심방세동(AF) 심전도 비교 그림: "
                    "대한부정맥학회 공식 환자교육 자료를 사용하도록 "
                    "고정한 위치입니다. 연구용 재게시 권한 확인 후 "
                    "실제 이미지 파일을 이 위치에 삽입하세요."
                )
    ...
    st.caption(
        "개인 진단·처방·약물 용량 변경·시술 결정은 제공하지 않습니다."
    )
